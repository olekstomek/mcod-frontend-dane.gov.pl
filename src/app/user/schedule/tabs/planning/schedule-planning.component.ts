...
export class SchedulePlanningComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.schedulePlanningService.isTableShouldBeReloaded()
            .pipe(
                take(1),
    ...
    changeScheduleStatus(status: boolean) {
        this.schedulePlanningService.updateScheduleStatus((this.dataSource as UserSchedule).userScheduleId, status)
            .subscribe(() => {
    ...
    blockSchedule(status: boolean) {
        this.schedulePlanningService.blockSchedule(status)
            .subscribe(() => {
     ...
     */
    openNewSchedule() {
        this.schedulePlanningService.openNewSchedule()
            .subscribe(schedule => {
    ...
    private configureRepresentativeScheduleView() {
        ...
        this.schedulePlanningService
            .getSchedulesItemForCurrentSchedule(this.isAdmin ? Number(this.activatedRoute.snapshot.paramMap.get('representativeId')) :
                undefined)
            .subscribe(schedule => {
                this.dataSource = schedule;
                this.config = this.getRepresentativeScheduleViewConfig();
    ...
    private getRepresentativeScheduleViewConfig() {
        ...
        return new ScheduleTableConfig.builder()
            .default()
            .withLeftButton(this.isAdmin || ((!isCompleted && !isBlocked) && !this.isAdmin) ? new ButtonConfig('Dodaj zgłoszenie', ['add-new']) : undefined)
            .withActions(this.isAdmin || (!isCompleted && !this.isAdmin) && (!isBlocked && !this.isAdmin) ?

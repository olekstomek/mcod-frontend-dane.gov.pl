...
export class CsrfMiddleware implements NestMiddleware {
    ...
    use(req: Request, res: Response, next: Function) {
        ...
        this.addCsrfTokenCookie(req, res)
            .subscribe(() => {
                    next();
                },
                () => next());
    ...
    addCsrfTokenCookie(req: Request, res: Response): Observable<any> {
        ...
        return this.getHeadersForCsrf(req)
    ...
    private getHeadersForCsrf(req: Request) {
        ...
        const pingURL = `${req.get('host').includes('localhost') ? hostWithProtocol + '/api' : req.protocol + '://' + 'api.' + req.get('host')}/ping`;
        console.log(pingURL);
        return this.httpService.get(pingURL)
            .pipe(

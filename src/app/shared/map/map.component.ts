...
export class MapComponent implements AfterViewInit, OnInit, OnDestroy {
    ...
    leafletOptions = {
        layers: [
            this.leafletService.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', this.mapOptions)
        ],
        worldCopyJump: true
    };
    ...
    ngOnInit() {
        ...
        this.filter$ = this.datasetService.resourceFilterChanged$.pipe(
        ...
        this.activatedRoute.queryParamMap.pipe(
            switchMap(() => this.datasetService.getGeoData(this.resourceId, {noData: 'true'}))
        )
            .subscribe(response => {

                const bounds = (response.meta.aggregations && response.meta.aggregations.bounds) ?
                    response.meta.aggregations.bounds : this.polandBounds;
    ...
    prepareNewDataAndMap(data: any) {
        ...
        this.linesAndPolygonsShapes = data.data
            .filter(element => element.attributes.shape.type !== 'Point')
    ...
    displayUserPreferences(event) {
        ...
        if (!event && this.userSettings) {
            this.datasetService
                .getGeoData(this.resourceId, this.buildParamsObject())
                .subscribe(response => this.prepareNewDataAndMap(response));
    ...
    private clearMapView() {
        ...
        allObjects
            .filter(element => !element.classList.contains('distanceCircle'))
            .map(element => element.parentNode.removeChild(element));
    }

    private setMapBoundsString(): string {
        const density = this.zoomToDensity();
        return `${this.mapBounds.getNorthWest().wrap().lng},${this.mapBounds.getNorthWest().wrap().lat},${this.mapBounds.getSouthEast().wrap().lng},${this.mapBounds.getSouthEast().wrap().lat},${density},${this.aggregationThreshold}`;
    }
    ...
    private markPointUserLocation() {
        if (this.userLocation) {
            this.userLocationMarker = this.leafletService.marker(this.userLocation.getCenterLocation(),
                this.userLocation.getUserLocationPointProperties())
                .addTo(this.map);
        }
    }

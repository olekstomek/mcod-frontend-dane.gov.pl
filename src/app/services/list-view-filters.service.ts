...
export class ListViewFiltersService {
    ...
    addTermsSuffix(name: string): string {
        return (name === AggregationFilterNames.CATEGORY ||
            name === AggregationFilterNames.CATEGORIES ||
            name === AggregationFilterNames.INSTITUTION) ? name + '[id][terms]' : name + '[terms]';
    }
     ...
     */
    updateParams(updatedParams: PageParams | BasicPageParams, method: QueryParamsHandling | null = 'merge',
                 basicParams: BasicPageParams, paramsIn: PageParams) {
        // default sort
     ...
     * @return {Params}
     */
    setSelectedFilters(queryParams: PageParams, dateFields: string [], options: Params, selectedFilters: Params,
                       params: BasicPageParams): Params {
        if (!options) {
            return;
        }
        ...
        for (const key in filters) {
            if (filters[key]) {
                if (filters[key].length) {
                    if (key === AggregationOptionType.CATEGORY || key === AggregationOptionType.CATEGORIES) {
    ...
    private translateVisualizationTypes(filtersByVisualizationTypes: IAggregationProperties[]) {
        ...
        const translateKeys = ['VisualizationTypesFilters.Map', 'VisualizationTypesFilters.Chart', 'VisualizationTypesFilters.None',

...
export class TourDataService extends RestService {
    ...
    getTours(): Observable<Array<Tour>> {
        if (!this.getTours$) {
            const params = new HttpParams()
                .append('include', 'item');
            this.getTours$ = this.get(ApiConfig.guides, params)
                .pipe(
                    map(this.mapResponseToTour()),
                    shareReplay()
                );
        }
    ...
    getTourById(id: string): Observable<Tour> {
        return this.getTours()
            .pipe(map(tours => tours.find(tour => tour.id === id)));
    }
    ...
    getRoutesWhereTourShouldButtonVisible(): Observable<{ [key: string]: boolean }> {
        if (!this.routesWithVisibleButton$) {
            this.routesWithVisibleButton$ = this.getTours()
                .pipe(
                    map(tours => {
                        const routes = {};
                        tours.forEach(tour => {
